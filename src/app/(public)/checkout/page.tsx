 ...
 * Cart item line
 * ------------------------------------------------------------------------ */
function CartItemLine({ item }: { item: CartItemView }) {
  const queryClient = useQueryClient();
  const { message } = AntdApp.useApp();
    ...
    >
      ...
      >
        ...
        <button
          ...
          disabled={
            update.isPending ||
            qty >= Math.min(10, item.book.stockQuantity)
          }
          onClick={() =>
            scheduleUpdate(
              Math.min(10, item.book.stockQuantity, qty + 1),
            )
          }
          style={stepperButtonStyle}
        >
...
function CheckoutInner() {
  ...
  const placeOrder = useMutation({
    ...
    onSuccess: (order) => {
      ...
      modal.success({
        ...
        onOk: () => router.push(`/orders/${order.id}?placed=1`),
      });
    },
    onError: (err) => {
  ...
  const total = cart.subtotal + displayShipping;
    ...
    <div style={{ paddingBottom: 40 }}>
      ...
      <Row gutter={[28, 28]}>
        <Col xs={24} lg={15}>
          ...
          >
            {cart.items.map((it) => (
              <CartItemLine key={it.id} item={it} />
            ))}
            <div
              style={{
                display: 'flex',
            ...
            {effectiveAddress ? (
              ...
              >
                ...
                <div style={{ color: 'var(--color-text)', fontSize: 14 }}>
                  ...
                  {effectiveAddress.ward}, {effectiveAddress.district},{' '}
                  {effectiveAddress.province}
                </div>
              </div>
            ) : (
          ...
          >
            ...
            >
              *Lưu ý: hệ thống tự áp dụng gói tiêu chuẩn cho đơn COD ở giai
              đoạn này.
            </div>
          </StepCard>
        ...
        <Col xs={24} lg={9}>
          <div style={{ position: 'sticky', top: 96 }}>
            ...
            >
              ...
              >
                <Tooltip title="Sẽ ra mắt">
                  <Input placeholder="Mã voucher" disabled />
                </Tooltip>
                <Tooltip title="Sẽ ra mắt">
